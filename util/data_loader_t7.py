...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        vid = f"data\\features\\medvidqa\\{record['vid']}"
        # 디버깅: vid와 video_features의 키 확인
        #print(f"Checking vid: '{vid}', Type: {type(vid)}")
        #print(f"Available keys: {list(self.video_features.keys())[:10]}")  # 첫 10개 출력

        if vid not in self.video_features:
            raise KeyError(f"Key '{vid}' not found in video_features.")
