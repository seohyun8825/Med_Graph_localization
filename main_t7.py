...
if configs.mode.lower() == 'train':
    ...
    for epoch in range(configs.epochs):
        ...
        for data in tqdm(train_loader, total=num_train_batches, desc='Epoch %3d / %3d' % (epoch + 1, configs.epochs)):
            ...
            # compute logits
            start_logits, end_logits= model(word_ids, char_ids, vfeats, video_mask, query_mask)
            # compute loss
            loc_loss = model.compute_loss(start_logits, end_logits, s_labels, e_labels)

            projected_vfeats = model.video_proj(vfeats)  # (batch_size, num_segments, dim)
            mean_projected_vfeats = torch.mean(projected_vfeats, dim=1)  # (batch_size, dim)
            query_features = model.embedding_net(word_ids, char_ids).mean(dim=1)
            ...
            if global_step % num_train_batches == 0:
                print('\nEpoch: %2d | Step: %5d | total_loss: %.2f | loc_loss: %.2f' % (
                epoch + 1, global_step, total_loss, loc_loss), flush=True)
